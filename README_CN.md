...
如需排查崩溃等异常问题，请使用 `--debug` 参数启动程序：

```sh
joycon2_connector.exe --debug
```

程序会在可执行文件所在目录生成 `joycon2_connector_debug.log` 日志文件。该日志会记录扫描/连接手柄时的详细蓝牙错误（Windows 蓝牙错误码、GATT 状态、ATT 协议错误、HRESULT 信息）。反馈崩溃或连接问题时请附带该日志。
