...
程序会显示连接失败的阶段、具体原因以及 Windows、GATT 或 ViGEm 错误代码。对于有明确处理方法的错误，可以点击“可能解决方案 ⓘ”。常见情况包括：

| 提示 | 常见原因 | 建议处理 |
|------|----------|----------|
| 蓝牙适配器未就绪或不可用 | 蓝牙关闭、适配器/驱动异常，常见代码 `HRESULT 0x800710DF` | 重新开关蓝牙；在设备管理器中重新启用或更新蓝牙适配器 |
| 未在规定时间内发现手柄 | 手柄未广播、距离过远、电量不足或进入连接冷却 | 先关闭手柄，开始扫描后持续按住配对键；靠近电脑；必要时等待几分钟 |
| 已发现手柄但无法建立蓝牙连接 | 配对键过早松开、连接中断、信号或驱动异常 | 重新扫描并持续按住配对键；删除旧配对记录；重新启用蓝牙适配器 |
| 无法访问蓝牙服务或输入通知 | 权限、旧配对记录或 Windows 蓝牙栈异常 | 删除旧配对记录，检查蓝牙权限，然后重新配对 |
| 无法创建虚拟手柄 | ViGEmBus 缺失、版本不匹配或虚拟手柄槽已满 | 安装/修复 ViGEmBus，重启电脑，关闭其他虚拟手柄程序 |

协议错误或内部异常可能没有可靠的通用解决方案，因此界面只显示失败阶段和原始错误代码，不显示解决方案按钮。
